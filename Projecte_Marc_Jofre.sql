...
CREATE DATABASE IF NOT EXISTS Projecte_Marc_Jofre;
...
-- Crear usuari normal
CREATE USER 'normal_user'@'localhost' IDENTIFIED BY 'user';

-- Donar tots els privilegis a l'administrador sobre la base de dades
GRANT ALL PRIVILEGES ON Projecte_Marc_Jofre.* TO 'admin_user'@'localhost';

-- Donar privilegis limitats a l'usuari normal (exemple: només SELECT, INSERT)
GRANT SELECT, INSERT ON Projecte_Marc_Jofre.* TO 'normal_user'@'localhost';

-- Aplicar els canvis
FLUSH PRIVILEGES;

-- Crear la funció per comptar les delegacions de cada empresa
DELIMITER $$

CREATE FUNCTION count_branches(company_name VARCHAR(255))
RETURNS INT
BEGIN
   ...
   RETURN IFNULL(branch_count, 0);
END$$

DELIMITER ;

-- Crear un trigger per eliminar delegacions quan s'elimina una empresa
DELIMITER $$

CREATE TRIGGER delete_company_branches
BEFORE DELETE ON Companies
FOR EACH ROW
BEGIN
   -- Eliminar totes les delegacions associades a l'empresa que s'està eliminant
